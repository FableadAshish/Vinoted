...
const Shimmer = ({ }) => {
    return (
        ...
        <View style={{ width: "100%" }}>
            <ShimmerPlaceHolder style={{ marginLeft: 10, height: 30, width: "60%", borderRadius: 10, marginVertical: 10 }} />
            <View style={[{ width: "90%", alignSelf: "center", height: 200, flexDirection: "row", justifyContent: "space-between", marginHorizontal: 5, marginBottom: 20, }]}>
            ...
            <ShimmerPlaceHolder style={{ height: 50, width: '90%', borderTopRightRadius: 30, borderBottomRightRadius: 30, marginVertical: 10 }} />
...
class ProductDetail extends Component {
    ...
    Store = async () => {
        ...
        http.get(`sommelier/product/${form.product_id}`).then(res => {
                    ...
                    />
                        ...
                        >
                            ...
                            <Icon name={isLike ? "heart" : "hearto"} style={{ color: isLike ? "red" : "gray" }} />
                                ...
                                <View style={{
                                    flexDirection: "row", marginHorizontal: 5, marginBottom: 20, flex: 1,
                                    marginVertical: 1,
                                    backgroundColor: '#F1FAEE',
                                    // elevation: 1,
                                    borderRadius: 5,
                                    ...
                                    <View style={{ flexDirection: 'column', flex: 0.7 }}>
                                        <View style={{ marginVertical: 5, }}>
                                            <Text style={[styles.textheading, { fontWeight: "700" }]}>{ProductDetail.title}</Text>
                                        ...
                                        {!isEmpty(event) && event.is_started == 1 && <View style={{ marginVertical: 5 }}>
                                            ...
                                            <Text style={{ fontFamily: sofiaFont, fontSize: 20, fontWeight: "700", color: primaryColor }}>{moment(form.created_at).format("DD MMM,YYYY")}</Text>
                                        </View>}
                                ...
                                <View style={{ flexGrow: 1, backgroundColor: white, }}>
                                        ...
                                        <View style={{ flexDirection: 'row', width: "95%", flex: 1, alignSelf: "center" }}>
                                            <View style={{ flexDirection: 'column', flex: 0.5, paddingHorizontal: 10 }}>
                                                <View style={{ marginVertical: 5 }}>
                                                    ...
                                                    <Text style={{ fontFamily: sofiaFont, fontSize: 15, color: primaryColor }}>{ProductDetail.type}</Text>
                                                </View>
                                                <View style={{ marginVertical: 5 }}>
                                                    <Text style={{ color: "gray" }}>Region</Text>
                                                    <Text style={{ fontFamily: sofiaFont, fontSize: 15, color: primaryColor }}>{ProductDetail.region}</Text>
                                                </View>
                                            </View>
                                    ...
                                    <View style={[styles.view, { marginHorizontal: 5, backgroundColor: white }]}>
                    ...
                    />
                        : (
                            (!isEmpty(event) && event.is_started == 1 &&
                                <TouchableOpacity
                                    onPress={() => !isEmpty(event) && event.is_started == 1 && this.props.navigation.navigate("ChooseProduct", { Testing: form, event: event })}
                                    style={{
