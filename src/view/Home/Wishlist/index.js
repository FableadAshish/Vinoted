...
class Index extends Component {
    ...
    Store = async () => {
        ...
        http.get(`sommelier/myfavouriterating?page=${this.state.page}`).then(res => {
            console.log("response Wishlist..", res)
            // this.setState({Wishlist:res.data,loading: false,refreshing: false})
            this.setState({
                Wishlist: this.state.page === 0 ? res.data.data : [...this.state.Wishlist, ...res.data.data]
                , loading: false,
    ...
    _renderEmptyComponent() {
        return !this.state.loading && isEmpty(this.state.Wishlist) && <FLEC text="Currently No Wines Have Been Favourited"
                                    ...
                                    if(!isNull(item.product)) {
                                        return (  
                                            ...
                                            onPressView={() =>  this.props.navigation.navigate("ChooseProduct", { Testing: item, event: item.event, isFromAdditional: true })}
                                                ...
                                                subtitle= {!isNull(item.product) && moment(item.product.created_at).format("DD/MM/YYYY") }    
                                                imagelist={!isNull(item.product) && item.product.Imagesrc}
