...
import {
    ...
    secondryColor,
    primaryColor,
    cardBackground, sofiaFont, MoreButton, textColor, calendarlist
...
const EventRequest = ({ subtitle, areaname, time,  doller, onPressMore, discription, more, item, isLike, title, date, imagelist, onPress, }) => {
   
    return (
        <TouchableOpacity onPress={onPress}>
            <View style={{ height: 80,overflow:"hidden" ,width: "100%", 
            alignSelf: "center", marginHorizontal: 5, marginVertical: 5, 
            justifyContent: "center", backgroundColor: calendarlist, 
            flexDirection: "row", borderRadius: 5,borderColor:primaryColor,borderWidth:1 }}>
                <View style={{ flex: 0.2, justifyContent: "center" ,paddingHorizontal:10,backgroundColor:"white"}}>
                    <View style={{ justifyContent: "center",alignItems:"center", paddingHorizontal: 5, height: 60, width: 60,  backgroundColor: white, }}>
                        {/* <Image style={{ height: 50, width: 50, borderRadius: 25 }} source={{ uri: imagelist }} /> */}
                        {/* {(() => this.sub(moment(item.products.created_at).format('DD'),'th'))()} */}
                        <Text style={{ color: primaryColor,fontSize:20 }}>{moment(item.date).format('DD')}<Text style={{fontSize:10,height:20}}>th</Text></Text>
                        <Text style={{ color: primaryColor,fontSize:20,marginTop:-10 }}>{moment(item.date).format('MMM')}</Text>
                    </View>
                </View>
                <View style={{ flex: 0.8, paddingHorizontal: 5, marginHorizontal: 5, marginTop: 3, justifyContent: "center" }}>
                    <Text numberOfLines={1} style={[styles.text, { fontSize: 15, width: "100%", color: white, textTransform: "capitalize", paddingVertical: 2 }]}>{title} </Text>
                    {areaname && <Text numberOfLines={3} style={[styles.text, { fontSize: 12, width: "100%", color: "lightgray", textTransform: "capitalize", paddingVertical: 2 }]}>{areaname}</Text>}
                    {/* {date &&
    ...
    Store = async () => {
        ...
        // http.get(`sommelier/eventproductnorating?page=${this.state.page}`).then(res => {
            ...
            this.setState({
                ...
                // ,
                Products: this.state.page === 0 ? res.data.page.data : [...this.state.Products, ...res.data.page.data]
                , 
                 loading: false,
                        ...
                        {this.state.loading && isEmpty(this.state.Products) ?
                            ...
                            (
                                <View style={{ marginBottom: 50}}>
                                    <Text style={[styles.textheading, { marginBottom: 10 }]}>Tasting Calender</Text>
                                    <FlatList
                                        ...
                                        renderItem={({ item }) => (
                                            // <View>
                                                <EventRequest
                                                    ...
                                                    onPressMore={() => this.props.navigation.navigate("ChooseProduct", { Testing: item })}
                                                    // more={"Tasting"}
