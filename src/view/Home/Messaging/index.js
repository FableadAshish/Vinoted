...
const Shimmer = ({ }) => {
    return (
        ...
        <View style={[{ height: 100, borderRadius: 5, backgroundColor: white, marginVertical: 5 }]}>
            <View style={{ height: 100, width: width - 20, justifyContent: "center", backgroundColor: white, elevation: 1, flexDirection: "row", alignSelf: "center" }}>
...
class Index extends Component {
    ...
    Store = async () => {
        ...
        http.get(`chatlist?page=${this.state.page}`).then(res => {
            console.log('chatres', res);
            this.setState({
                UsersList: this.state.page === 0 ? res.data : [...this.state.UsersList, ...res.data]
                , loading: false,
    ...
    _renderEmptyComponent() {
        return !this.state.loading && isEmpty(this.state.UsersList) && <FLEC text="No data available"
    ...
    SearchFilterFunction(text) {
        ...
        if (text == "") {
            ...
            http.get(`sommelier/suppliers?search=${text}`).then(res => {
                console.log("response search", res)
                this.setState({ searchdata: res.data.page, loading: false, refreshing: false })
    ...
    createSession(item) {
        ...
        // this.setState({ loading: true })
        // http.post(`session/create`, { friend_id: item.id }).then(res => {
        //     console.log("response create Session", res)
        //     // this.props.navigation.navigate("Chat",{item:item,Session:res.data})
        //     this.setState({ loading: false, refreshing: false, text: "" }, () => this.props.navigation.navigate("Chat", { item: item, Session: res.data }))
        ...
        //     this.setState({ loading: false, errors, refreshing: false });
        // });
        // () => this.props.navigation.navigate("Chat",{ChatItem:item}) 
                ...
                {this.state.loading && isEmpty(this.state.UsersList) ?
                    ...
                    >
                        <View style={{ marginHorizontal: 5,}}>
                            ...
                            {this.state.text == "" ? <View style={styles.view}>
                                <FlatList
                                    showsVerticalScrollIndicator={false}
                                    ...
                                    renderItem={({ item }) => {
                                        console.log("chate....UsersList",item.session.respective_user.profile)
                                        return (
                                            <TouchableOpacity onPress={() => this.createSession(item)}>
                                                <CardList
                                                    onPress={() => this.createSession(item)}
                                                    imagelist={!isEmpty(item.profile) ? item.profile : "https://www.freeiconspng.com/uploads/customers-icon-3.png"}
                                                    subtitle={item.message != '' ? item.message.content : ""}
                                                    ShowCount={item.session.unreadCount>0 && item.session.unreadCount}
                                                    // ShowCount={2}
                                                    title={!isEmpty(item.session.respective_user.profile) && item.session.respective_user.profile.first_name}
                                                    date={ //item.message!=''?
                                                        (moment(currentDate).format("DD MMM YYYY") == moment(item.message.created_at).format("DD MMM YYYY")
                                                            ?
                                                            moment(item.message.created_at).format("HH:mm")
                                                            :
                                                            moment(item.message.created_at).format("DD MMM YYYY"))
                                                        //:""
                                                    }
                                        ...
                                        renderItem={({ item }) => {
                                            ...
                                            return (
                                                <TouchableOpacity onPress={() => this.createSession(item)}>
                                                    <CardList
                                                        onPress={() => this.createSession(item)}
                                                        imagelist={!isEmpty(item.profile) ? item.profile.photo : "https://www.freeiconspng.com/uploads/customers-icon-3.png"}
                                                         subtitle={item.email}
                                                        title={!isEmpty(item.profile) && item.profile.first_name}
                                                         date={ //item.message!=''?
                                                             (moment(currentDate).format("DD MMM YYYY")== moment(item.created_at).format("DD MMM YYYY")
                                                         ?
