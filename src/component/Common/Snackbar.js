...
import { Text, View, StyleSheet,Animated,SafeAreaView,TouchableWithoutFeedback } from 'react-native';
import { error, success, warning,sofiaFont } from '../../style/variables';
...
export default class Snackbar extends React.Component {
  ...
  show(message = '',type = 'error',duration = 3000){
    ...
    Animated.timing(this.state.opacity,{
      toValue:1, duration:255, useNativeDriver:true
    }).start(({finished}) => {
  ...
  render() {
    ...
    return (
      <SafeAreaView pointerEvents="box-none" style={styles.swrapper}>
        <Animated.View  pointerEvents="box-none" accessibilityLiveRegion="polite"
         style={[styles.scontainer,{backgroundColor,opacity:this.state.opacity,
         transform:[{scale: !hidden ? this.state.opacity.interpolate({
                        inputRange: [0, 1], outputRange: [0.9, 1] }):1}]
         }]}
        >
          <Text style={[styles.scontent, { marginRight: actionText ? 0 : 16 ,color}]}>
            {this.state.message}
          </Text>
          {
            actionText ?   <TouchableWithoutFeedback onPress={this._onPress}><View style={styles.actionRoot}>
               <View style={styles.actionContainer}> 
                <Text numberOfLines={1} style={styles.actionText}>{actionText.toUpperCase()}</Text>
              </View>
